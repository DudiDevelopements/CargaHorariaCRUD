...
namespace CargaHorariaCRUD.WebHelper {
    public class Sessao : ISessao {
        ...
        public object? GetSessao() {
            ...
            if(usuarioLogado.Contains("Envios"))
                return JsonConvert.DeserializeObject<UsuarioModel>(usuarioLogado);

            // Verifica se é um AdmModel
            else if(usuarioLogado.Contains("Senha"))
                return JsonConvert.DeserializeObject<AdmModel>(usuarioLogado);

            // Se não for nenhum dos dois manda erro
            else
                throw new Exception("Tipo de objeto desconhecido na sessão");
        }
    }
}
